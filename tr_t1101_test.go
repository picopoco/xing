...
func TestF현물_호가_조회_t1101(t *testing.T) {
	...
	값, 에러 := F현물_호가_조회_t1101(lib.F임의_종목().G코드())
	lib.F테스트_에러없음(t, 에러)
	...
	lib.F테스트_같음(t, 값.M매도호가수량합, 매도호가수량합)
	lib.F테스트_같음(t, 값.M매수호가수량합, 매수호가수량합)

	if lib.F한국증시_정규시장_거래시간임() {
		lib.F테스트_참임(t, 값.M수신시간.After(time.Now().Add(-1*lib.P10분)),
			time.Now().Add(-1*lib.P10분).Format("15:04:06"), 값.M수신시간.Format("15:04:06"))
	...
	lib.F테스트_같음(t, 값.M동시호가_구분, xt.P동시호가구분_장중, xt.P동시호가구분_시간외, xt.P동시호가구분_동시)
