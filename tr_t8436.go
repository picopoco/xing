...
func F주식종목조회_t8436(시장_구분 lib.T시장구분) (응답값_모음 []*S현물_종목조회_응답_반복값, 에러 error) {
	...
	질의값 := lib.New질의값_문자열(TR조회, TR현물_종목_조회_t8436, 시장구분_문자열)
...
func New현물_종목조회_응답_반복값_모음(b []byte) (값 *S현물_종목조회_응답_반복값_모음, 에러 error) {
	...
	값 = new(S현물_종목조회_응답_반복값_모음)
	값.M배열 = make([]*S현물_종목조회_응답_반복값, 수량, 수량)
