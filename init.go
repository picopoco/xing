...
func init() {
	...
	//접속리스트 표시 없이 최초 접속을 강제 접속 처리후 자동 로그인(옵션)
	//
	//==> 옵션 설정 방법
	//
	//xingAPI 설치폴더(일반적으로 C:\eBEST\xingAPI) 내 'xingapi.ini' 파일을 열어 아래 내용 추가
	//
	//[API]
	//
	//CHECK_LOGINLIST=1
	...
	//입니다.
	//
	//	다만, 해당 옵션이용시 맨 처음에 로그인 한 접속이 종료되기 때문에, 종료시키는 프로그램의 미종료된 프로그램여부는 확인하실 수 없습니다.
...
func tr동작_확인(ch완료 chan lib.T신호) {
	...
	if len(tr코드별_초당_전송_제한) == 0 {
		tr코드별_초당_전송_제한[TR시간_조회_t0167] = lib.New전송_권한(TR시간_조회_t0167, 5, lib.P1초)
	}
...
func f전일_당일_설정() (에러 error) {
	...
	질의값_기간별_조회.M구분 = TR조회
	질의값_기간별_조회.M코드 = TR현물_기간별_조회_t1305
...
func f초기화_TR전송_제한() (에러 error) {
	...
	TR코드_모음 := []string{
		TR현물_정상_주문_CSPAT00600,
		TR현물_정정_주문_CSPAT00700,
		TR현물_취소_주문_CSPAT00800,
		TR시간_조회_t0167,
		TR체결_미체결_조회_t0425,
		TR현물_호가_조회_t1101,
		TR현물_시세_조회_t1102,
