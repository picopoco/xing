...
func TestF바이트_변환값_해석(t *testing.T) {
	...
	원본값_모음 := []interface{}{
		...
		new(S질의값_정상_주문_CSPAT00600), new(S질의값_정정_주문_CSPAT00700), new(S질의값_취소_주문_CSPAT00800), new(S질의값_현물_전일당일_분틱_조회_t1310),
		new(S질의값_현물_기간별_조회_t1305), new(lib.S질의값_단일종목_연속키), new(S질의값_증시주변자금추이_t8428),
		...
		new(S현물_호가_조회_응답), new(S현물_시세조회_응답), new(S_ETF_현재가_조회_응답),
		new(S현물_시간대별_체결_응답), new(S현물_시간대별_체결_응답_헤더), new(S현물_시간대별_체결_응답_반복값),
		new(S현물_시간대별_체결_응답_반복값_모음),
		new(S현물_기간별_조회_응답_t1305), new(S현물_기간별_조회_응답_헤더_t1305), new(S현물_기간별_조회_응답_반복값_t1305),
		new(S현물_기간별_조회_응답_반복값_모음_t1305),
		...
		new(S증시_주변자금추이_응답), new(S증시_주변자금추이_응답_헤더), new(S증시_주변자금추이_응답_반복값),
		new(S증시_주변자금추이_응답_반복값_모음),
