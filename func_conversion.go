...
func F바이트_변환값_해석(바이트_변환값 *lib.S바이트_변환) (해석값 interface{}, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 해석값 = nil }}.S실행()
	...
	case P자료형_S콜백_메시지_및_에러:
		...
		lib.F에러체크(바이트_변환값.G값(s))
		return s, nil
	case P자료형_S현물_정상주문_응답:
		s := new(S현물_정상주문_응답)
		lib.F에러체크(바이트_변환값.G값(s))
		return s, nil
	case P자료형_S현물_정상주문_응답1:
		s := new(S현물_정상주문_응답1)
		lib.F에러체크(바이트_변환값.G값(s))
		return s, nil
	case P자료형_S현물_정상주문_응답2:
		s := new(S현물_정상주문_응답2)
		lib.F에러체크(바이트_변환값.G값(s))
		return s, nil
	case P자료형_S현물_정정주문_응답:
		s := new(S현물_정정주문_응답)
		lib.F에러체크(바이트_변환값.G값(s))
		return s, nil
	case P자료형_S현물_정정주문_응답1:
		s := new(S현물_정정주문_응답1)
		lib.F에러체크(바이트_변환값.G값(s))
		return s, nil
	case P자료형_S현물_정정주문_응답2:
		s := new(S현물_정정주문_응답2)
		lib.F에러체크(바이트_변환값.G값(s))
		return s, nil
	case P자료형_S현물_취소주문_응답:
		s := new(S현물_취소주문_응답)
		lib.F에러체크(바이트_변환값.G값(s))
		return s, nil
	case P자료형_S현물_취소주문_응답1:
		s := new(S현물_취소주문_응답1)
		lib.F에러체크(바이트_변환값.G값(s))
		return s, nil
	case P자료형_S현물_취소주문_응답2:
		s := new(S현물_취소주문_응답2)
		lib.F에러체크(바이트_변환값.G값(s))
		return s, nil
