...
func TestF재무_순위_종합_t3341(t *testing.T) {
	...
	for _, 값 := range 값_모음 {
		...
		lib.F테스트_참임(t, math.Abs(실수값(값.ROE)) < 10000, 값.M종목코드, 값.ROE)
		lib.F테스트_참임(t, math.Abs(실수값(값.PER)) < 10000, 값.M종목코드, 값.PER)
		lib.F테스트_참임(t, math.Abs(실수값(값.PBR)) < 10000, 값.M종목코드, 값.PBR)
