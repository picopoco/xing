...
func TestCSPAT00600현물_정상주문(t *testing.T) {
	...
	질의값_매수.M계좌번호 = 계좌번호
	질의값_매수.M계좌_비밀번호 = "" // 모의투자에서는 계좌 비밀번호를 체크하지 않음.
	질의값_매수.M종목코드 = 종목.G코드()
	질의값_매수.M주문수량 = 주문수량
	...
	for i:=0 ; i<반복_횟수 ; i++ {
		// 매수
		매수_응답값, 에러 := F현물_정상주문_CSPAT00600(질의값_매수)
		...
		lib.F테스트_참임(t, 매수_응답값.M응답2.M주문시각.After(p1분전), 매수_응답값.M응답2.M주문시각, p1분전)
		lib.F테스트_참임(t, 매수_응답값.M응답2.M주문시각.Before(p1분후), 매수_응답값.M응답2.M주문시각, p1분후)
		lib.F테스트_참임(t, 매수_응답값.M응답2.M주문번호 > 0)
	

		// 매도 (매수에서 매도로 바뀐 것 빼고는 앞선 주문과 동일함.)
		매도_응답값, 에러 := F현물_정상주문_CSPAT00600(질의값_매도)

		lib.F테스트_에러없음(t, 에러)
		lib.F테스트_다름(t, 매도_응답값.M응답1, nil)
		...
		lib.F테스트_다름(t, 매도_응답값.M응답2, nil)
		lib.F테스트_같음(t, 매도_응답값.M응답2.M종목코드, 종목.G코드())
		lib.F테스트_참임(t, 매도_응답값.M응답2.M주문시각.After(p1분전), 매도_응답값.M응답2.M주문시각, p1분전)
		lib.F테스트_참임(t, 매도_응답값.M응답2.M주문시각.Before(p1분후), 매도_응답값.M응답2.M주문시각, p1분후)
