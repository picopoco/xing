...
func New현물_취소_주문_응답(b []byte) (값 *S현물_취소_주문_응답, 에러 error) {
	...
	값.M응답1, 에러 = New현물_취소_주문_응답1(b[:SizeCSPAT00800OutBlock1])
	lib.F확인(에러)

	값.M응답2, 에러 = New현물_취소_주문_응답2(b[SizeCSPAT00800OutBlock1:])
	lib.F확인(에러)

	return 값, nil
}

func New현물_취소_주문_응답1(b []byte) (s *S현물_취소_주문_응답1, 에러 error) {
...
func New현물_취소_주문_응답2(b []byte) (s *S현물_취소_주문_응답2, 에러 error) {
	defer lib.S예외처리{M에러: &에러, M함수: func() { s = nil }}.S실행()
	...
	if lib.F2문자열_공백제거(g.OrdNo) == "" { // 주문 에러발생시 공백 문자열이 수신됨.
		return nil, lib.New에러("New현물_취소_주문_응답2() : 주문번호 생성 에러.")
	}
	...
	s.M신용거래_코드 = f2신용거래_구분(T신용거래_구분(lib.F2문자열_공백제거(g.MgntrnCode)))
