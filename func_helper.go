...
func f데이터_복원(대기_항목 *대기_항목_C32, 수신값 *lib.S바이트_변환) error {
	...
	case TR현물_기간별_조회, TR현물_당일_전일_분틱_조회, TR_ETF_시간별_추이, TR현물_차트_틱, TR증시_주변_자금_추이:
...
func f데이터_복원_반복_조회(대기_항목 *대기_항목_C32, 수신값 *lib.S바이트_변환) error {
	if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.1.0", 대기_항목.식별번호, 대기_항목.TR코드, 대기_항목.데이터_수신, 대기_항목.메시지_수신, 대기_항목.응답_완료, 대기_항목.에러) }

	완전값 := new(S헤더_반복값_일반형)

	if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.1.1") }
	...
	if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.2.2") }

	switch 변환값 := 수신값.S해석기(F바이트_변환값_해석).G해석값_단순형().(type) {
	default:
		if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.2.3.1") }

		panic(lib.New에러("예상하지 못한 자료형 : '%v' '%v'", 변환값, 수신값.G자료형_문자열()))
	case error:
		if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.2.3.2") }

		체크(변환값.Error())
		return 변환값
	case I헤더_반복값_TR데이터:
		if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.2.3.3") }
	...
	case I헤더_TR데이터:
		if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.2.3.4") }

		완전값.M헤더 = 변환값
	case I반복값_모음_TR데이터:
		if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.2.3.5") }

		완전값.M반복값_모음 = 변환값
	}

	if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.2.4") }

	대기_항목.대기값 = 완전값

	if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.2.5") }

	if 완전값.M헤더 != nil && 완전값.M반복값_모음 != nil {
		if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.2.6.1") }

		대기_항목.데이터_수신 = true
	} else {
		if 대기_항목.TR코드 == TR현물_차트_틱 { lib.F체크포인트("5.2.6.2") }

		대기_항목.데이터_수신 = false
	}

	if 대기_항목.TR코드 == TR현물_차트_틱 {
		lib.F체크포인트("5.2.7", 대기_항목.식별번호, 대기_항목.TR코드, 대기_항목.데이터_수신, 대기_항목.메시지_수신, 대기_항목.응답_완료, 대기_항목.에러)
	}
...
func f처리_가능한_TR코드(TR코드 string) bool {
	...
	case TR시간_조회, TR현물_정상_주문, TR현물_정정_주문, TR현물_취소_주문,
		...
		TR현물_차트_틱, TR증시_주변_자금_추이, TR현물_종목_조회:
		return true
