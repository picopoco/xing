...
func TestF주식종목조회_API용_t8436(t *testing.T) {
	...
	for _, 응답값 := range 값_모음 {
		...
		// 상한가 예상값 계산의 예외가 너무 많아서 건너뜀.
		//호가단위, 에러 := lib.F최소_호가단위by시장구분_현재가(종목.G시장구분(), 응답값.M전일가)
		//lib.F테스트_에러없음(t, 에러)
		...
		lib.F테스트_참임(t, 응답값.M상한가 == 0 || 응답값.M상한가 > 응답값.M하한가)

		lib.F메모("예상하지 못한 값에 대해서 게시판 질문 후 답변 기다리는 중.")
		if 응답값.M종목코드 != "115530" {
			lib.F테스트_참임(t, 응답값.M상한가 == 0 || 응답값.M전일가 == 0 || 응답값.M상한가 > 응답값.M전일가)
		}

		lib.F테스트_참임(t, 응답값.M상한가 == 0 || 응답값.M상한가 > 응답값.M기준가)
		...
		if 응답값.M전일가 != 0 {
			lib.F테스트_참임(t, 응답값.M하한가 == 0 || 응답값.M하한가 <= 응답값.M전일가,
				응답값.M종목코드, 응답값.M하한가, 응답값.M전일가)
		}

		lib.F테스트_참임(t, 응답값.M하한가 == 0 || 응답값.M하한가 <= 응답값.M기준가,
			응답값.M종목코드, 응답값.M하한가, 응답값.M기준가)
		lib.F테스트_같음(t, 응답값.M증권그룹,
			xt.P증권그룹_주식, xt.P증권그룹_예탁증서,
			xt.P증권그룹_증권투자회사_뮤추얼펀드, xt.P증권그룹_Reits종목,
			xt.P증권그룹_상장지수펀드_ETF, xt.P증권그룹_선박투자회사,
			xt.P증권그룹_인프라투융자회사, xt.P증권그룹_해외ETF,
			xt.P증권그룹_해외원주, xt.P증권그룹_ETN)
		//lib.F테스트_같음(t, 응답값.M기업인수목적회사여부, true, false)
	}
}
