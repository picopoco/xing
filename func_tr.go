...
func F시각_조회_t0167() (시각 time.Time, 에러 error) {
	...
	질의값 := lib.S질의값_기본형{TR구분: TR조회, TR코드: TR시간_조회}
	시각 = 에러체크(F질의_단일TR(질의값, lib.P20초)).(time.Time)

	return 시각, nil
}

func F현물_정상주문_CSPAT00600(질의값 *S질의값_정상주문) (응답값 *S현물_정상주문_응답, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 응답값 = nil }}.S실행()

	응답값 = 에러체크(F질의_단일TR(질의값)).(*S현물_정상주문_응답)

	return 응답값, nil
}

func F현물_정정주문_CSPAT00700(질의값 *S질의값_정정주문) (응답값 *S현물_정정주문_응답, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 응답값 = nil }}.S실행()

	응답값 = 에러체크(F질의_단일TR(질의값)).(*S현물_정정주문_응답)

	return 응답값, nil
}

func F현물_취소주문_CSPAT00800(질의값 *S질의값_취소주문) (응답값 *S현물_취소주문_응답, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 응답값 = nil }}.S실행()

	응답값 = 에러체크(F질의_단일TR(질의값)).(*S현물_취소주문_응답)
...
func F현물_당일전일_분틱_조회_t1310(종목코드 string, 당일전일_구분 T당일전일_구분, 분틱_구분 T분틱_구분,
	...
	응답값_모음 = make([]*S현물_전일당일분틱조회_응답_반복값, 0)
	연속키 := ""
...
func F증시주변자금추이_t8428(시장_구분 lib.T시장구분, 추가_옵션_모음 ...interface{}) (응답값_모음 []*S증시주변자금추이_응답_반복값, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 응답값_모음 = nil }}.S실행()
