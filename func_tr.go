...
func F기간별_주가_조회_t1305(종목코드 string, 일주월_구분 xt.T일주월_구분) (
	...
	for {
		...
		응답값, ok := i응답값.(*xt.S현물_기간별_조회_응답)
		if !ok {
			panic(lib.New에러("예상하지 못한 자료형 : '%T'", 응답값))
		}
...
func F현물_당일전일_분틱_조회_t1310(종목코드 string, 당일전일_구분 xt.T당일전일_구분, 분틱_구분 xt.T분틱_구분,
	...
	for {
		...
		응답값, ok := i응답값.(*xt.S현물_전일당일분틱조회_응답)
		if !ok {
			panic(lib.New에러("예상하지 못한 자료형 : '%T'", 응답값))
		}
...
func F주식종목조회_t8436(시장_구분 lib.T시장구분) (응답값_모음 []*xt.S현물_종목조회_응답_반복값, 에러 error) {
	...
	질의값 := lib.New질의값_문자열(lib.TR조회, xt.TR현물_종목_조회, 시장구분_문자열)
	i응답값, 에러 := F질의_단일TR(질의값)
	lib.F체크포인트(lib.F자료형_문자열_단순형(i응답값))

	응답값_모음 = i응답값.(*xt.S현물_종목조회_응답_반복값_모음).M배열
