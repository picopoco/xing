...
func F현물_정상주문_CSPAT00600(질의값 *xt.S질의값_정상주문) (응답값 *xt.S현물_정상주문_응답, 에러 error) {
	...
	응답값 = 에러체크(F단일TR(질의값)).(*xt.S현물_정상주문_응답)
...
func F현물_정정주문_CSPAT00700(질의값 *xt.S질의값_정정주문) (응답값 *xt.S현물_정정주문_응답, 에러 error) {
	...
	응답값 = 에러체크(F단일TR(질의값)).(*xt.S현물_정정주문_응답)
...
func F현물_취소주문_CSPAT00800(질의값 *xt.S질의값_취소주문) (응답값 *xt.S현물_취소주문_응답, 에러 error) {
	...
	응답값 = 에러체크(F단일TR(질의값)).(*xt.S현물_취소주문_응답)

	return 응답값, nil
}

func F현물_호가_조회_t1101(종목코드 string) (응답값 *xt.S현물호가조회_응답, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 응답값 = nil }}.S실행()
	...
	응답값 = 에러체크(F단일TR(질의값)).(*xt.S현물호가조회_응답)

	return 응답값, nil
}

func F현물_시세_조회_t1102(종목코드 string) (응답값 *xt.S현물시세조회_응답, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 응답값 = nil }}.S실행()
	...
	응답값 = 에러체크(F단일TR(질의값)).(*xt.S현물시세조회_응답)

	return 응답값, nil
}

func F기간별_주가_조회_t1305(종목코드 string, 일주월_구분 xt.T일주월_구분) (
	응답값_모음 []*xt.S기간별_주가_조회_응답_반복값, 에러 error) {
	...
	for {
		...
		응답값 := 에러체크(F단일TR(질의값)).(*xt.S기간별_주가_조회_응답)
		에러체크(F질의(질의값).G값(0, &응답값))

		연속키 = 응답값.M헤더.M연속키
		응답값_모음 = append(응답값_모음, 응답값.M반복값_모음...)

		lib.F조건부_패닉(응답값.M헤더.M수량 != int64(len(응답값.M반복값_모음)),
			"반복값 수량 불일치. '%v', '%v'", 응답값.M헤더.M수량, len(응답값.M반복값_모음))
...
func F현물_당일전일_분틱_조회_t1310(종목코드 string, 당일전일_구분 xt.T당일전일_구분, 분틱_구분 xt.T분틱_구분,
	...
	for {
		...
		질의값.M종목코드 = 종목코드
		질의값.M당일전일구분 = string(당일전일_구분)
		질의값.M분틱구분 = string(분틱_구분)
		질의값.M종료시각 = 종료시각
		질의값.M연속키 = 연속키

		응답값 := 에러체크(F단일TR(질의값)).(*xt.S현물_전일당일분틱조회_응답)
		에러체크(F질의(질의값).G값(0, &응답값))
...
func ETF_시간별_추이_t1902(종목코드 string) (응답값_모음 []*xt.S_ETF시간별_추이_응답_반복값, 에러 error) {
	...
	for {
		...
		응답값 := 에러체크(F단일TR(질의값)).(*xt.S_ETF시간별_추이_응답)
		에러체크(F질의(질의값).G값(0, &응답값))
...
func F증시주변자금추이_t8428(시장_구분 lib.T시장구분) (응답값_모음 []*xt.S증시주변자금추이_응답_반복값, 에러 error) {
	...
	for {
		...
		응답값 := 에러체크(F단일TR(질의값)).(*xt.S증시주변자금추이_응답)
		에러체크(F질의(질의값).G값(0, &응답값))
...
func F주식종목조회_t8436(시장_구분 lib.T시장구분) (응답값_모음 []*xt.S주식종목조회_응답_반복값, 에러 error) {
	defer lib.S에러패닉_처리기{M에러_포인터: &에러, M함수: func() { 응답값_모음 = nil }}.S실행()
	...
	case lib.P시장구분_코스닥:
		시장구분_문자열 = "2"
	default:
		panic(lib.F2문자열("예상하지 못한 시장 구분값 : '%v'", 시장_구분))
	}

	질의값 := lib.New질의값_문자열()
	질의값.TR구분 = lib.TR조회
	질의값.TR코드 = xt.TR현물_종목_조회
	질의값.M문자열 = 시장구분_문자열

	panic("TODO")
	//응답값 := 에러체크(F단일TR(질의값)).(*xt.S???)
	//에러체크(F질의(질의값).G값(0, &응답값))
	//
	//연속키 = 응답값.M헤더.M연속키
	//응답값_모음 = append(응답값_모음, 응답값.M반복값_모음...)
