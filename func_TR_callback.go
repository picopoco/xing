...
func f콜백_데이터_복원(대기_항목 *c32_콜백_대기_항목, 수신값 *lib.S바이트_변환) error {
	switch 대기_항목.TR코드 {
	case TR시간_조회_t0167, TR체결_미체결_조회_t0425,
		TR현물_호가_조회_t1101, TR현물_시세_조회_t1102, TR현물_종목_조회_t8436:
		대기_항목.대기값 = 수신값.S해석기(F바이트_변환값_해석).G해석값_단순형() // 단순 질의
		대기_항목.데이터_수신 = true
	case TR현물_정상_주문_CSPAT00600, TR현물_정정_주문_CSPAT00700, TR현물_취소_주문_CSPAT00800, TR기업정보_요약_t3320:
		return f데이터_복원_이중_응답(대기_항목, 수신값) // 이중 응답 질의
	case TR재무순위_종합_t3341, TR현물_기간별_조회_t1305, TR현물_당일_전일_분틱_조회_t1310,
		TR관리_불성실_투자유의_조회_t1404, TR투자경고_매매정지_정리매매_조회_t1405,
		TR_ETF_시간별_추이_t1902, TR현물_차트_틱_t8411, TR현물_차트_분_t8412, TR현물_차트_일주월_t8413,
		TR증시_주변_자금_추이_t8428:
