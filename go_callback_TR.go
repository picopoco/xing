	...
	for {
		...
		default:
			...
			func() {
				...
				에러 = 소켓REP_TR콜백.S회신_Raw(수신_메시지, 변환_형식, lib.P신호_OK)
				lib.F조건부_실행(에러 != nil, lib.F에러_출력, 에러)
			}()
		}
	}
}
...
func f콜백_TR데이터_처리기(값 I콜백) (에러 error) {
	...
	switch 값.G콜백() {
	case P콜백_TR데이터:
		에러체크(f데이터_복원(대기_항목, 값.(*S콜백_TR데이터).M데이터))
		//lib.F체크포인트(값.G콜백(), 대기_항목.식별번호, 값.(*S콜백_TR데이터).M데이터.G자료형_문자열())
	case P콜백_메시지_및_에러:
		변환값 := 값.(*S콜백_메시지_및_에러)

		if f에러_발생(TR코드, 변환값.M코드, 변환값.M내용) {
			체크("에러", 대기_항목.식별번호, 변환값.M코드, 변환값.M내용)
			대기_항목.에러 = lib.New에러("%s : %s", 변환값.M코드, 변환값.M내용)
			//lib.F체크포인트(값.G콜백(), 대기_항목.식별번호, 대기_항목.코드, 대기_항목.메시지)
		} else {
			//체크("메시지", 대기_항목.식별번호, 변환값.M코드, 변환값.M내용)
			대기_항목.메시지_수신 = true
		}
	case P콜백_TR완료:
		대기_항목.응답_완료 = true
		//lib.F체크포인트(값.G콜백(), 대기_항목.식별번호, 대기_항목.데이터_수신, 대기_항목.응답_완료)
	case P콜백_타임아웃:
