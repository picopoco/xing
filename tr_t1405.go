...
func F투자경고_조회_t1405(시장_구분 lib.T시장구분, 투자경고_질의_구분 T투자경고_질의_구분) (응답값_모음 []*S투자경고_조회_응답_반복값_t1405, 에러 error) {
	defer lib.S예외처리{M에러: &에러, M함수: func() { 응답값_모음 = nil }}.S실행()

	응답값_모음 = make([]*S투자경고_조회_응답_반복값_t1405, 0)
	연속키 := ""

	lib.F확인(F접속_확인())

	for {
		질의값 := new(S질의값_투자경고_조회_t1405)
		질의값.S질의값_기본형 = lib.New질의값_기본형(TR조회, TR투자경고_매매정지_정리매매_조회_t1405)
		질의값.M시장_구분 = 시장_구분
...
type S질의값_투자경고_조회_t1405 struct {
	*lib.S질의값_기본형
	M시장_구분      lib.T시장구분
...
type S투자경고_조회_응답_t1405 struct {
	M헤더     *S투자경고_조회_응답_헤더_t1405
	M반복값_모음 *S투자경고_조회_응답_반복값_모음_t1405
}

func (s *S투자경고_조회_응답_t1405) G헤더_TR데이터() I헤더_TR데이터 {
	return s.M헤더
}

func (s *S투자경고_조회_응답_t1405) G반복값_TR데이터() I반복값_모음_TR데이터 {
	return s.M반복값_모음
}

type S투자경고_조회_응답_헤더_t1405 struct {
	M연속키 string
}

func (s *S투자경고_조회_응답_헤더_t1405) G헤더_TR데이터() I헤더_TR데이터 {
	return s
}

type S투자경고_조회_응답_반복값_모음_t1405 struct {
	M배열 []*S투자경고_조회_응답_반복값_t1405
}

func (s *S투자경고_조회_응답_반복값_모음_t1405) G반복값_모음_TR데이터() I반복값_모음_TR데이터 {
	return s
}

type S투자경고_조회_응답_반복값_t1405 struct {
	M종목코드     string
	M종목명      string
	M현재가      int64
	M전일대비구분   T전일대비_구분
...
func New투자경고_조회_응답_반복값_모음_t1405(b []byte) (값_모음 * S투자경고_조회_응답_반복값_모음_t1405, 에러 error) {
	...
	값_모음 = new(S투자경고_조회_응답_반복값_모음_t1405)
	값_모음.M배열 = make([]*S투자경고_조회_응답_반복값_t1405, 수량, 수량)

	for i, g := range g_모음 {
		g = new(T1405OutBlock1)
		lib.F확인(binary.Read(버퍼, binary.BigEndian, g))

		값 := new(S투자경고_조회_응답_반복값_t1405)
		값.M종목코드 = lib.F2문자열(g.Shcode)
		값.M종목명 = lib.F2문자열_EUC_KR_공백제거(g.Hname)
		값.M현재가 = lib.F2정수64_단순형_공백은_0(g.Price)
