...
func F현물_당일전일_분틱_조회_t1310(종목코드 string, 당일전일_구분 T당일전일_구분, 분틱_구분 T분틱_구분,
	...
	defer func() {
		일자 := lib.F조건부_시간(당일전일_구분 == P당일전일구분_당일, F당일(), F전일())
		수량 = len(응답값_모음_역순)
	...
	for {
		...
		질의값.M구분 = TR조회
		질의값.M코드 = TR현물_당일_전일_분틱_조회_t1310
		질의값.M종목코드 = 종목코드
		질의값.M당일전일구분 = 당일전일_구분
		질의값.M분틱구분 = 분틱_구분
...
// t1310 전일당일분틱조회
type S질의값_현물_전일당일_분틱_조회_t1310 struct {
	*lib.S질의값_단일_종목
...
func NewT1310InBlock(질의값 *S질의값_현물_전일당일_분틱_조회_t1310) (g *T1310InBlock) {
	g = new(T1310InBlock)
...
func New질의값_현물_전일당일_분틱_조회() *S질의값_현물_전일당일_분틱_조회_t1310 {
