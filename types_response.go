...
func (s *S헤더_반복값_일반형) G값(TR코드 string) interface{} {
	...
	case TR현물_기간별_조회_t1305:
		값 := new(S현물_기간별_조회_응답_t1305)
		값.M헤더 = s.M헤더.(*S현물_기간별_조회_응답_헤더_t1305)
		값.M반복값_모음 = s.M반복값_모음.(*S현물_기간별_조회_응답_반복값_모음_t1305)
		return 값
	...
	case TR관리_불성실_투자유의_조회_t1404:
		값 := new(S관리종목_조회_응답_t1404)
		값.M헤더 = s.M헤더.(*S관리종목_조회_응답_헤더_t1404)
		값.M반복값_모음 = s.M반복값_모음.(*S관리종목_조회_응답_반복값_모음_t1404)
		return 값
	case TR투자경고_매매정지_정리매매_조회_t1405:
		값 := new(S투자경고_조회_응답_t1405)
		값.M헤더 = s.M헤더.(*S투자경고_조회_응답_헤더_t1405)
		값.M반복값_모음 = s.M반복값_모음.(*S투자경고_조회_응답_반복값_모음_t1405)
		return 값
	...
	case TR재무순위_종합_t3341:
		값 := new(S재무순위_응답_t3341)
		값.M헤더 = s.M헤더.(*S재무순위_응답_헤더_t3341)
		값.M반복값_모음 = s.M반복값_모음.(*S재무순위_응답_반복값_모음_t3341)
		return 값
