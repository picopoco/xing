...
func (s *S헤더_반복값_일반형) G값(TR코드 string) interface{} {
	...
	case TR증시_주변_자금_추이:
		값 := new(S증시주변자금추이_응답)
		값.M헤더 = s.M헤더.(*S증시주변자금추이_응답_헤더)
		값.M반복값_모음 = s.M반복값_모음.(*S증시주변자금추이_응답_반복값_모음)
		return 값
	}
}
...
func (s *S증시주변자금추이_응답_헤더) G헤더_TR데이터() I헤더_TR데이터 { return s }

// t8428 증시주변자금추이 응답 반복값
type S증시주변자금추이_응답_반복값 struct {
	M일자       time.Time
	M지수       float64
	M전일대비_구분  T전일대비_구분
	M전일대비_등락폭 float64
...
type S증시주변자금추이_응답_반복값_모음 struct {
