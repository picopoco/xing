...
func TestCSPAT00700현물_정정_주문(t *testing.T) {
	...
	lib.F대기(lib.P300밀리초) // 이게 있어야 에러가 발생하지 않음. 게시판 질문 중.
	...
	for {
		...
		case !ok:
			continue
		case 실시간_정보.M주문번호 != 정상주문_응답값.M응답2.M주문번호:
			continue
		}
		...
		case RT현물_주문_접수:
			// OK
		case RT현물_주문_거부:
			lib.F문자열_출력("정상 주문 거부됨 : '%v'", 정상주문_응답값.M응답2.M주문번호)
			t.FailNow()
		default:
	...
	*/
	...
	for i := 0; i < 반복_횟수; i++ {
		...
		for {
			...
			체크(i, ok, 실시간_정보.M원_주문번호, 원주문번호,  실시간_정보.M주문번호, 정정주문_응답값.M응답2.M주문번호)

			switch {
			case !ok:
				체크(i, "!ok")
				continue
			case 실시간_정보.M주문번호 != 정정주문_응답값.M응답2.M주문번호:
				체크(i, "주문번호 불일치")
				continue
			...
			switch 실시간_정보.RT코드 {
			case RT현물_주문_거부:
				lib.F문자열_출력("정정 주문 거부됨 : '%v'", 정상주문_응답값.M응답2.M주문번호)
				t.FailNow()
			case RT현물_주문_체결, RT현물_주문_취소:
				lib.F문자열_출력("예상하지 못한 TR코드 : '%v'", 실시간_정보.RT코드)
				t.FailNow()
			case RT현물_주문_접수:
				체크(i, "접수")
