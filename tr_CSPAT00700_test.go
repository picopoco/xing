...
func TestCSPAT00700_현물_정정_주문_질의값(t *testing.T) {
	_, ok := interface{}(new(xt.S질의값_정정_주문_CSPAT00700)).(lib.I질의값)
	lib.F테스트_참임(t, ok)
}
...
func TestCSPAT00700_현물_정정_주문(t *testing.T) {
	...
	질의값 := xt.New질의값_정상_주문()
	질의값.M구분 = xt.TR주문
	질의값.M코드 = xt.TR현물_정상_주문_CSPAT00600
	질의값.M계좌번호 = 계좌번호
	...
	for i := 0; i < 반복_횟수; i++ {
		...
		정정주문_응답값, 에러 := TR_CSPAT00700_현물_정정주문_(질의값_정정주문)
		lib.F테스트_에러없음(t, 에러)
	...
	취소주문_응답값, 에러 := TR_CSPAT00800_현물_취소주문(질의값_취소주문)
	lib.F테스트_에러없음(t, 에러)
