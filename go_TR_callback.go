...
func f콜백_TR데이터_처리기(값 I콜백) (에러 error) {
	...
	if TR코드 == TR현물_차트_틱 { lib.F체크포인트(식별번호, TR코드, 값.G콜백(), 대기_항목.데이터_수신, 대기_항목.메시지_수신, 대기_항목.응답_완료, 대기_항목.에러) }
	...
	if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("0") }

	lib.F조건부_패닉(식별번호 == 0 || 대기_항목 == nil || TR코드 == "", "대기항목 없음.")

	if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("1") }

	lib.F조건부_패닉(!f처리_가능한_TR코드(TR코드), "처리 불가 TR코드 : '%v'", TR코드)

	if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("2") }

	lib.F조건부_패닉(대기_항목 == nil, "TR 식별번호 '%v' : nil 대기항목.", 식별번호)

	if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("3") }

	대기_항목.Lock()
	defer 대기_항목.Unlock()

	if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("4") }

	switch 값.G콜백() {
	case P콜백_TR데이터:
		if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("5.1") }
		...
		if 에러 = f데이터_복원(대기_항목, 값.(*S콜백_TR데이터).M데이터); 에러 != nil && 대기_항목.에러 == nil {
			...
			case strings.Contains(에러.Error(), "New현물_정정주문_응답2() : 주문번호 생성 에러"),
				...
				return
			default:
				if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("5.1.에러") }
			...
			//	lib.F체크포인트("5.1.데이터 복원 완료 후 return")
			//}
	...
	case P콜백_메시지_및_에러:
		...
		if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("5.2", 변환값.M코드, 변환값.M내용) }
		...
		대기_항목.메시지_수신 = true
	case P콜백_TR완료:
		if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("5.3") }

		대기_항목.응답_완료 = true
	case P콜백_타임아웃:
		if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("5.4") }

		대기_항목.에러 = lib.New에러("타임아웃.")
		//lib.F체크포인트("타임아웃.")
	default:
		if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("5.5") }

		체크()
		panic(lib.New에러("예상하지 못한 경우. 콜백 구분값 : '%v', 자료형 : '%T'", 값.G콜백(), 값))
	}

	if TR코드 == TR현물_차트_틱 {  lib.F체크포인트("6") }

	// TR응답 데이터 수신 및 완료 확인이 되었는 지 확인.
	switch {
	case 대기_항목.에러 != nil && 대기_항목.메시지_수신 && 대기_항목.응답_완료:
		if TR코드 == TR현물_차트_틱 { lib.F체크포인트("7.1", 식별번호, TR코드, 값.G콜백(), 대기_항목.데이터_수신, 대기_항목.메시지_수신, 대기_항목.응답_완료, 대기_항목.에러) }

		대기소_C32.S회신(식별번호)
	case !대기_항목.데이터_수신, !대기_항목.응답_완료, !대기_항목.메시지_수신:
		if TR코드 == TR현물_차트_틱 { lib.F체크포인트("7.2", 식별번호, TR코드, 값.G콜백(), 대기_항목.데이터_수신, 대기_항목.메시지_수신, 대기_항목.응답_완료, 대기_항목.에러) }

		return
	default:
		if TR코드 == TR현물_차트_틱 { lib.F체크포인트("7.3", 식별번호, TR코드, 값.G콜백(), 대기_항목.데이터_수신, 대기_항목.메시지_수신, 대기_항목.응답_완료, 대기_항목.에러) }
