...
func NewS기업정보_요약_응답2(b []byte) (값 *S기업정보_요약_응답2, 에러 error) {
	...
	값.PBR = lib.F2실수_소숫점_추가_단순형(g.Pbr, 2)
	값.ROA = lib.F2실수_소숫점_추가_단순형(g.Roa, 2)
	값.ROE = lib.F2실수_소숫점_추가_단순형(g.Roe, 2)
	값.EBITDA = lib.F2실수_소숫점_추가_단순형(g.Ebitda, 2)
	값.EVEBITDA = lib.F2실수_소숫점_추가_단순형(g.Evebitda, 2)
	값.M액면가 = lib.F2실수_소숫점_추가_단순형(g.Par, 2)
	값.SPS = lib.F2실수_소숫점_추가_단순형(g.Sps, 2)
	값.CPS = lib.F2실수_소숫점_추가_단순형(g.Cps, 2)
	값.BPS = lib.F2실수_단순형(g.Bps)
	값.T_PER = lib.F2실수_소숫점_추가_단순형(g.Tper, 2)
