...
func F시각_조회_t0167() (ch응답 chan *xt.S시각_조회_응답_t0167) {
	...
	ch응답 = make(chan *xt.S시각_조회_응답_t0167, 1)
...
func f시각_조회_작업(인수 interface{}) {
	var 에러 error
	var ch응답 chan *xt.S시각_조회_응답_t0167

	defer lib.S예외처리{M에러: &에러, M함수: func() {
		if ch응답 != nil {
			ch응답 <- xt.New시각_조회_응답_t0167(time.Time{}, 에러)
		}
	}}.S실행()

	ch응답 = 인수.(chan *xt.S시각_조회_응답_t0167)
...
func F현물_체결_미체결_조회_t0425(계좌번호, 비밀번호, 종목코드 string, 체결_구분 lib.T체결_구분,
	...
	응답값_모음 = make([]*xt.S현물_체결_미체결_조회_응답_반복값_t0425, 0)
	연속키 := ""

	F접속_확인()

	for {
		질의값 := new(xt.S질의값_체결_미체결_조회_t0425)
		...
		값, ok := i응답값.(*xt.S현물_체결_미체결_조회_응답_t0425)
		lib.F조건부_패닉(!ok, "예상하지 못한 자료형 : '%T'", i응답값)
...
func F기간별_주가_조회_t1305(종목코드 string, 일주월_구분 xt.T일주월_구분, 추가_옵션_모음 ...interface{}) (
	...
	연속키 := ""
	응답값_모음 = make([]*xt.S현물_기간별_조회_응답_반복값_t1305, 0)
	...
	defer func() { // 순서 거꾸로 뒤집기.
		...
		응답값_모음 = make([]*xt.S현물_기간별_조회_응답_반복값_t1305, 수량)
	...
	for {
		...
		값, ok := i응답값.(*xt.S현물_기간별_조회_응답_t1305)
		lib.F조건부_패닉(!ok, "예상하지 못한 자료형 : '%T'", i응답값)
...
func F현물_당일전일_분틱_조회_t1310(종목코드 string, 당일전일_구분 xt.T당일전일_구분, 분틱_구분 xt.T분틱_구분,
	...
	응답값_모음_역순 := make([]*xt.S현물_전일당일분틱조회_응답_반복값, 0)
	연속키 := ""

	defer func() {
		일자 := lib.F조건부_시간(당일전일_구분 == xt.P당일전일구분_당일, F당일(), F전일())
		수량 = len(응답값_모음_역순)
		응답값_모음 = make([]*xt.S현물_전일당일분틱조회_응답_반복값, len(응답값_모음_역순))
...
func F관리종목_조회_t1404(시장_구분 lib.T시장구분, 관리_질의_구분 xt.T관리_질의_구분) (응답값_모음 []*xt.S관리종목_조회_응답_반복값_t1404, 에러 error) {
	...
	for {
		...
		값, ok := i응답값.(*xt.S관리종목_조회_응답_t1404)
		lib.F조건부_패닉(!ok, "예상하지 못한 자료형 : '%T'", i응답값)
...
func F투자경고_조회_t1405(시장_구분 lib.T시장구분, 투자경고_질의_구분 xt.T투자경고_질의_구분) (응답값_모음 []*xt.S투자경고_조회_응답_반복값_t1405, 에러 error) {
	...
	for {
		...
		값, ok := i응답값.(*xt.S투자경고_조회_응답_t1405)
		lib.F조건부_패닉(!ok, "예상하지 못한 자료형 : '%T'", i응답값)
...
func ETF_시간별_추이_t1902(종목코드 string, 추가_옵션_모음 ...interface{}) (응답값_모음 []*xt.S_ETF시간별_추이_응답_반복값, 에러 error) {
	...
	defer func() { // 순서 거꾸로 뒤집고, 종목코드 정보 및 누락된 시각 데이터 추가.
		...
		응답값_모음 = make([]*xt.S_ETF시간별_추이_응답_반복값, 수량)
...
func F재무_순위_종합_t3341(시장구분 lib.T시장구분, 재무순위_구분 xt.T재무순위_구분,
	...
	응답값_모음 = make([]*xt.S재무순위_응답_반복값_t3341, 0)
	연속키 := ""

	for {
		질의값 := xt.New질의값_재무순위_t3341()
		질의값.M시장구분 = 시장구분
...
func F현물_차트_틱_t8411(종목코드 string, 시작일자, 종료일자 time.Time, 추가_인수_모음 ...interface{}) (응답값_모음 []*xt.S현물_차트_틱_응답_반복값, 에러 error) {
	...
	응답값_모음 = make([]*xt.S현물_차트_틱_응답_반복값, 0)
	연속일자 := ""
	연속시간 := ""
...
func F현물_차트_30초_t8412(종목코드 string, 시작일자, 종료일자 time.Time, 추가_인수_모음 ...interface{}) (응답값_모음 []*xt.S현물_차트_분_응답_반복값, 에러 error) {
	...
	응답값_모음 = make([]*xt.S현물_차트_분_응답_반복값, 0)
	연속일자 := ""
	연속시간 := ""
...
func F현물_차트_일주월_t8413(종목코드 string, 시작일자, 종료일자 time.Time, 주기구분 xt.T일주월_구분,
	...
	응답값_모음 = make([]*xt.S현물_차트_일주월_응답_반복값, 0)
	연속일자 := ""
...
func F증시주변자금추이_t8428(시장_구분 lib.T시장구분, 추가_옵션_모음 ...interface{}) (응답값_모음 []*xt.S증시_주변자금추이_응답_반복값, 에러 error) {
	...
	응답값_모음 = make([]*xt.S증시_주변자금추이_응답_반복값, 0)
